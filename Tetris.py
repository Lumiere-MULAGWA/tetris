    ...
    while not done:
        ...
        for i in range(game.height):
            for j in range(game.width):
                pygame.draw.rect(screen, (178,190,181), [game.x + game.zoom * j, game.y + game.zoom * i, game.zoom, game.zoom], 1)
                if game.field[i][j] > 0:
...
pygame.display.set_caption("Tetris by lmr_lumiere")
run = True
while run:
    screen.fill((16, 57, 34 ))
    font = pygame.font.SysFont("Calibri", 70, bold=True)
    label = font.render('Press any key to begin!', True,(255,255,255))
